...
// ── Digits ───────────────────────────────────────────────────────────────
// normalizeDigits: convert any Arabic-Indic (٠-٩) or Persian (۰-۹) digits to
// Western 0-9. Applied at render level so the UI shows Western digits
// everywhere (surah/ayah numbers, counts, badges, metadata, indicators).
const _WESTERN_DIGITS: Record<string,string> = {
  '٠':'0','١':'1','٢':'2','٣':'3','٤':'4','٥':'5','٦':'6','٧':'7','٨':'8','٩':'9',
  '۰':'0','۱':'1','۲':'2','۳':'3','۴':'4','۵':'5','۶':'6','۷':'7','۸':'8','۹':'9',
};
function normalizeDigits(input: string | number | null | undefined): string {
  return String(input ?? '').replace(/[٠-٩۰-۹]/g, d => _WESTERN_DIGITS[d] ?? d);
}
// dg: render a number/string for display with Western digits.
const dg = (n: string | number) => normalizeDigits(n);
...
function formatAyahRef(ref:AyahRef|null|undefined, compact=false){
  if(!ref)return "";
  return compact ? `${dg(ref.surah)}:${dg(ref.ayah)}` : `سورة ${dg(ref.surah)} · آية ${dg(ref.ayah)}`;
}
    ...
    <div className="hpicker">
      ...
      {done.size > 0 && (
        <div className="hpicker-progress">
          ...
          <span className="hpicker-prog-lbl">{dg(done.size)}/60 حزب مكتمل</span>
        </div>
      )}
        ...
        {hizbsForJuz.map((h,i)=>{
          ...
          return (
            <div key={h.hizb_num} className={`hcard${isSel?" hcard-sel":""}${isDone?" hcard-done":""}${isSugg&&!isDone?" hcard-sugg":""}`}
              ...
              aria-label={`حزب ${dg(h.hizb_num)}${isDone?' — مكتمل':''}${isSugg&&!isDone?' — المقترح التالي':''}${isSel?' — محدد':''}`}
              aria-pressed={isSel}
              onClick={()=>onSelect(h)}
              ...
              <div className="hcard-range">
                <div className="hcard-point">
                  ...
                  <span className="hcard-ayah">آية {dg(h.start_ayah)}</span>
                </div>
                ...
                <div className="hcard-point">
                  ...
                  <span className="hcard-ayah">آية {dg(h.end_ayah)}</span>
                </div>
              </div>
      ...
      {selected&&(
        <div className="hpicker-sel-summary">
          ...
          <span>الحزب {dg(selected.hizb_num)} · الجزء {dg(selected.juz_num)}</span>
          <span className="hpicker-sel-range">
            {surahName(selected.start_surah)} {dg(selected.start_ayah)}
            &nbsp;—&nbsp;
            {surahName(selected.end_surah)} {dg(selected.end_ayah)}
          </span>
        </div>
      )}
    </div>
        ...
        {tab==='wird' && (<>
          ...
          <div className="hist-stats">
            <div className="hist-stat">
              <span className="hist-stat-n">{dg(total)}</span>
              <span className="hist-stat-l">حزب مكتمل</span>
            </div>
            <div className="hist-stat-div"/>
            <div className="hist-stat">
              <span className="hist-stat-n">{dg(pct)}%</span>
              <span className="hist-stat-l">من القرآن</span>
            </div>
            <div className="hist-stat-div"/>
            <div className="hist-stat">
              <span className="hist-stat-n">{dg(history.length)}</span>
            ...
            {Array.from({length:60},(_,i)=>i+1).map(n=>{
              ...
              return (
                <div key={n} className={`hist-cell${isDone?' hist-cell-done':''}`}
                  ...
                  {isDone ? <IcCheck s={9} c="var(--teal2)"/> : <span className="hist-cell-n">{dg(n)}</span>}
                </div>
              );
            })}
          </div>
                  ...
                  <div key={i} className="hist-entry">
                    <div className="hist-entry-left">
                      ...
                      <span className="hist-entry-what">
                        {hizbData ? `حزب ${dg(hizbData.hizb_num)} · ج${dg(hizbData.juz_num)}` :
          ...
          <div className="hifd-ring-row">
            <svg className="hifd-ring-svg" viewBox="0 0 80 80" width="80" height="80">
              ...
              <text x="40" y="36" textAnchor="middle" fill="var(--gold)"
                fontSize="13" fontWeight="700" fontFamily="var(--ff)">{dg(quranHifdPct)}%</text>
            ...
            <div className="hifd-ring-stats">
              <div className="hifd-rstat">
                <span className="hifd-rstat-n" style={{color:'var(--teal2)'}}>{dg(completedSurahs)}</span>
                <span className="hifd-rstat-l">سورة مكتملة</span>
              </div>
              <div className="hifd-rstat">
                <span className="hifd-rstat-n" style={{color:'var(--gold)'}}>{dg(inProgressSurahs)}</span>
                <span className="hifd-rstat-l">في التقدم</span>
              </div>
              <div className="hifd-rstat">
                <span className="hifd-rstat-n">{dg(totalHifdedAyahs)}</span>
                ...
                <div className="hifd-surah-top">
                  ...
                  <span className={`hifd-surah-pct${surahPct===100?' hifd-pct-done':''}`}>
                    {surahPct===100
                      ...
                      : <>{dg(hifded)}<span className="hifd-pct-sep">/</span>{dg(totalAyahs)} آية — {dg(surahPct)}%</>
                    }
                  </span>
                </div>
            ...
            <div className="arp-sw">
              <select className="arp-sel" value={min}
                ...
                {Array.from({length:total},(_,i)=>i+1).map(n=>(
                  <option key={n} value={n}>آية {dg(n)}</option>
                ))}
              </select>
      ...
      <div className="arp-bar-wrap">
        <div className="arp-bar">
          <div className="arp-fill" style={{
            insetInlineStart:`${whole?0:((min-1)/total)*100}%`,
          ...
          {total<=80 && Array.from({length:total},(_,i)=>i+1).map(n=>(
            <div key={n} className={`arp-tick${(whole||(n>=min&&n<=max))?" in":""}`}
              style={{insetInlineStart:`${((n-0.5)/total)*100}%`}}/>
          ))}
        </div>
        <div className="arp-blabels">
          <span>1</span>
          <span className="arp-binfo">{whole?`كاملة · ${dg(total)} آية`:`${dg(min)}←${dg(max)} · ${dg(cnt)} آية (${pct}%)`}</span>
          <span>{dg(total)}</span>
        </div>
      </div>
      <div className="arp-summary">
        <div className="arp-sc"><span>من</span><strong>{dg(whole?1:min)}</strong></div>
        <div className="arp-sc"><span>إلى</span><strong>{dg(whole?total:max)}</strong></div>
        <div className="arp-sc"><span>المجموع</span><strong>{dg(cnt)} آية</strong></div>
      </div>
    </div>
  );
}
  ...
  return (
    <div className="tf-panel">
      <div className="tf-header">
        <span style={{display:"flex",alignItems:"center",gap:6}}><IcScroll s={14} c="var(--gold2)"/> التفسير الميسر — آية {dg(ayahNum)}</span>
      ...
      <p className="tf-source">التفسير الميسر — نخبة من العلماء · <span className="fl">jsDelivr CDN</span></p>
    </div>
  );
}
  ...
  return (
    <div className="mq-panel">
      <div className="mq-hdr">
        <span className="mq-title" style={{display:"flex",alignItems:"center",gap:6}}><IcCrescent s={14} c="var(--gold2)"/> مقاصد الآية {dg(ayahNum)}</span>
...
function QuranTextPanel({ ayahs, surahNum, surahName, activeAyah, onAyahClick, scrollRef, notes, onNoteChange, onNoteAyahSelect }:{
  ...
  const copySelectedAyah = async()=>{
    ...
    const text = `${selectedAyahText.text}\n\nسورة ${name}، الآية ${dg(selectedAyah.ayah)}`;
    try {
        ...
        <div className="mushaf-inner">
          ...
          <div className="mushaf-hdr">
            ...
            <div className="mushaf-hdr-sub">
              {isMultiSurah
                ? `${dg(ayahs.length)} آية عبر ${dg(grouped.length)} سور`
                : firstAyah===lastAyah
                ? `الآية الكريمة ${dg(firstAyah)}`
                : `الآيات الكريمة ${dg(firstAyah)} – ${dg(lastAyah)}`}
            </div>
          </div>
                    ...
                    {group.ayahs.map(a=>{
                      ...
                      return (
                        <span key={key}
                          ...
                          role="button" tabIndex={0}
                          aria-label={`الآية ${dg(a.numberInSurah)}${isActive?' — قيد التلاوة':''}`}
                          aria-current={isActive?'true':undefined}
                          onClick={()=>handleClick(ref)}
                          onKeyDown={e=>{if(e.key==='Enter'||e.key===' '){e.preventDefault();handleClick(ref);}}}
                        >
                          {a.text}
                          <span className="mushaf-anum">{String.fromCodePoint(0x06DD)}{dg(a.numberInSurah)}</span>
...
function SyncPlayer({ url, filename, sizeKb, timings, onAyahChange, onSeekToAyah,
  ...
  const timingLabel=useCallback((t:AyahTiming)=>{
    ...
    return hasExplicitSurah ? formatAyahRef(ref,true) : dg(ref.ayah);
  },[timingRef]);
  ...
  return (
    <div className="splayer">
      ...
      <div className="sp-ctrl">
        <button className="sp-skip" onClick={()=>skip(-10)}><IcArrowR s={12} c="currentColor"/> 10ث</button>
        ...
        <button className="sp-skip" onClick={()=>skip(10)}>10ث <IcArrowL s={12} c="currentColor"/></button>
      </div>
        ...
        {autoReplay&&<>
          <div className="sp-rbtns">
            ...
            <span className="sp-rdone">
              {playing
                ? <>{dg(replayDone+1)}&nbsp;/&nbsp;{dg(replayCount)}</>
                : <>{dg(replayCount)}× تكرار</>
              }
            </span>
          </div>
        </>}
      ...
      <div className="hifd-progress">
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
          ...
          <span style={{display:"flex",alignItems:"center",gap:8}}>
            <span style={{fontSize:".78rem",fontWeight:700,color:"var(--gold2)"}}>آية {dg(idx+1)} / {dg(ayahs.length)}</span>
            {memorizedCount > 0 && <span className="hifd-mem-badge"><IcCheck s={11} c="var(--teal3)"/> {dg(memorizedCount)} محفوظة</span>}
          </span>
        </div>
      ...
      <div className="hifd-ayah-card">
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:10}}>
          <span style={{fontSize:".78rem",color:"var(--textD)",fontWeight:600}}>
            {surahName} · آية {dg(currentAyah.numberInSurah)}
          </span>
        ...
        <div className={`hifd-ayah-text${showText ? ' revealed' : ' blurred'}`}>
          {currentAyah.text}
          <span className="qnum">{String.fromCodePoint(0x06DD)}{dg(currentAyah.numberInSurah)}</span>
        </div>
        {!showText && !result && (
      ...
      {result && score !== null && (
        <div className="hifd-result">
          <div className={`hifd-score-badge${score>=80?' great':score>=50?' ok':' bad'}`}>
            <span className="hifd-score-pct">{dg(score)}%</span>
  ...
  return (
    <div className={`app${dark?" dark":" light"}${readingMode?" reading-mode":""}`}>
        ...
        <header className="hdr">
          <div className="hdr-inner">
            ...
            <button className="hist-btn" onClick={()=>setShowHistory(true)} title="سجل القراءة" aria-label="سجل القراءة">
              ...
              {completedHizbs.size>0&&<span className="hist-btn-badge">{dg(completedHizbs.size)}</span>}
            </button>
          </div>
        </header>
                ...
                <div className="sg" role="listbox" aria-label="قائمة السور">
                  {filtered.map((s,i)=>{
                    ...
                    <div key={s.id} className={`si${selS?.id===s.id?" sel":""}`} style={{"--idx":i} as any}
                      ...
                      <span className="si-n">{dg(s.id)}</span>
                      <div className="si-body"><span className="si-ar">{s.name_arabic}</span>
                        <span className="si-en"><bdi dir="ltr">{s.translated_name} · {dg(s.verses_count)} ayat</bdi></span></div>
                      <div className="si-right">
                        {hpct!=null && (
                          hpct===100
                            ? <span className="si-hifd-done">✓ محفوظة</span>
                            : <span className="si-hifd-pct">{dg(hpct)}%</span>
                        )}
          ...
          <div className={`wcard slide-${dir}`}>
            <div className="wcard-hdr"><span className="wcard-icon"><IcBeads s={30} c="var(--gold)"/></span>
              <div><div className="wcard-title">نطاق الآيات</div>
                <div className="wcard-sub">سورة <strong>{selS?.name_arabic}</strong> — {dg(selS?.verses_count??0)} آية</div></div>
            </div>
            <div className="wcard-body">
              ...
              <div>
                ...
                <div className="wcard-sub">
                  {selMode==='hizb'&&selHizb
                    ? <>حزب {dg(selHizb.hizb_num)} · جزء {dg(selHizb.juz_num)} · {RECITERS_META[selR!]?.nameAr}</>
                    : <>{selS?.name_arabic} · {RECITERS_META[selR!]?.nameAr} · آية {dg(dMin)}–{dg(dMax)}</>
                  }
                </div>
              </div>
                  ...
                  <div className="qtext-col qtext-full">
                    ...
                    {loadingText
                      ...
                      ? <div className="data-error data-error-block" role="alert">
                          ...
                          <div className="data-error-sub">خدمة <bdi dir="ltr">alquran.cloud</bdi> غير متاحة حالياً — تحقق من الاتصال</div>
                  ...
                  <div className="qs-fs-bar-inner">
                    <div className="qs-fs-meta">
                      <span className="qs-fs-title">
                        {selMode==='hizb'&&selHizb ? `حزب ${dg(selHizb.hizb_num)}` : (selS?.name_arabic??'—')}
                      </span>
                    ...
                    <div className="qs-fs-center">
                      <div className="qs-fs-btns">
                        <button className="qs-fs-skip" onClick={()=>fpControlsRef.current?.skip(-10)} title="رجوع 10 ثانية">
                        ...
                        <button className="qs-fs-skip" onClick={()=>fpControlsRef.current?.skip(10)} title="تقديم 10 ثانية">
                          <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round">
              ...
              <div className="fp-row">
                ...
                {fpAutoReplay&&(fpTikrarOpen ? (
                  ...
                  <button className="fp-rcbtn active" onClick={()=>setFpTikrarOpen(true)}
                    ...
                    {dg(fpReplayCount)}×{fpReplayScope==='ayah'?' آية':''}
                  </button>
                ))}
      ...
      <footer className="footer">
        ...
        <p className="footer-copy"><bdi dir="ltr">© {new Date().getFullYear()} <strong>Noureddine Achibane</strong></bdi> — جميع الحقوق محفوظة</p>
...
.srch{width:100%;background:var(--input-bg);border:1px solid var(--border);border-radius:11px;padding-block:11px;padding-inline-start:38px;padding-inline-end:36px;color:var(--text);font-family:var(--ff);font-size:.92rem;outline:none;direction:rtl;transition:border-color .2s;letter-spacing:.01em;line-height:1.5}
.srch:focus{border-color:var(--border2);box-shadow:0 0 0 3px rgba(201,168,76,.07)}.srch::placeholder{color:var(--textDD)}
.srch-x{position:absolute;inset-inline-end:10px;background:none;border:none;color:var(--textD);cursor:pointer;font-size:.75rem;padding:4px;transition:color .2s}
...
.arp-sel{width:100%;background:var(--input-bg);border:1px solid var(--border);border-radius:10px;padding-block:11px;padding-inline-start:14px;padding-inline-end:30px;color:var(--text);font-family:var(--ff);font-size:.92rem;appearance:none;-webkit-appearance:none;outline:none;cursor:pointer;transition:border-color .2s;direction:rtl;letter-spacing:.01em}
.arp-sel:focus{border-color:var(--border2);box-shadow:0 0 0 3px rgba(201,168,76,.07)}
.arp-arr{position:absolute;inset-inline-end:10px;top:50%;transform:translateY(-50%);color:var(--textD);font-size:.75rem;pointer-events:none}
...
.arp-fill{position:absolute;top:0;bottom:0;background:linear-gradient(90deg,rgba(201,168,76,.18),rgba(201,168,76,.38));border-radius:6px;transition:all .4s cubic-bezier(.22,.68,0,1.2);border-inline-start:2px solid rgba(201,168,76,.55)}
...
/* quran text */
.mushaf-text-wrap{overflow-y:auto;max-height:min(560px,calc(100dvh - 300px));padding-inline-end:2px;padding-bottom:var(--player-clearance);overscroll-behavior:contain}
...
.qnum{font-family:var(--ff);font-size:.74rem;color:var(--gold);vertical-align:middle;margin-inline-start:2px;opacity:.75}
...
.sp-tog-thumb{position:absolute;top:2px;inset-inline-start:2px;width:10px;height:10px;border-radius:50%;background:var(--textD);transition:all .2s}
.sp-toggle input:checked~.sp-tog-track .sp-tog-thumb{inset-inline-start:16px;background:var(--teal3)}
...
.sp-rdone{font-size:.68rem;color:var(--gold);background:rgba(201,168,76,.14);padding:3px 10px;border-radius:12px;font-weight:700;direction:rtl;margin-inline-start:auto;margin-inline-end:6px;border:1px solid rgba(201,168,76,.25);white-space:nowrap}
...
.footer-sub{font-size:.68rem;color:var(--textDD);letter-spacing:.01em}.fl{color:var(--teal3);direction:ltr;unicode-bidi:isolate}
...
.hist-btn-badge{position:absolute;top:-4px;inset-inline-end:-4px;min-width:16px;height:16px;border-radius:8px;background:var(--gold);color:#0c1020;font-size:.58rem;font-weight:700;display:flex;align-items:center;justify-content:center;padding:0 4px;font-family:var(--ff)}
...
.hcard-done-badge{font-size:.64rem;color:var(--teal3);background:rgba(42,157,143,.12);border:1px solid rgba(42,157,143,.2);border-radius:8px;padding:2px 7px;margin-inline-start:auto;display:flex;align-items:center;gap:3px}
.hcard-sugg-badge{font-size:.64rem;color:var(--gold);background:rgba(201,168,76,.1);border:1px solid rgba(201,168,76,.2);border-radius:8px;padding:2px 7px;margin-inline-start:auto;animation:pulse 2s infinite}
`}</style>
    </div>
  );
}
